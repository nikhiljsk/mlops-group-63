...
class LoggingService:
    ...
    async def log_prediction(self, request_data: Dict[str, Any], prediction_result: Dict[str, Any]):
        ...
        try:
            async with self._lock:
                ...
                logger.debug(f"Logged prediction: {prediction_result.get('prediction')}")
                
        except Exception as e:
            logger.error(f"Failed to log prediction: {e}")
    
    async def log_batch_prediction(self, request_data: List[Dict[str, Any]], batch_result: Dict[str, Any]):
        ...
        try:
            async with self._lock:
                ...
                for i, prediction_result in enumerate(batch_result.get("predictions", [])):
                    individual_request = request_data[i] if i < len(request_data) else {}
                    ...
                    """, (
                        ...
                        prediction_result.get("confidence", 0.0),
                        prediction_result.get("model_version", "unknown"),
                        batch_result.get("total_processing_time_ms", 0.0) / len(batch_result.get("predictions", [1])),
                        batch_result.get("batch_size", 1)
                    ))
                
                self.connection.commit()
                logger.debug(f"Logged batch prediction: {batch_result.get('batch_size')} samples")
    ...
    async def log_api_metrics(self, endpoint: str, method: str, status_code: int, 
                            response_time_ms: float, request_size: int = 0, response_size: int = 0):
        ...
        try:
            async with self._lock:
                ...
                return {
                    ...
                    "avg_processing_time_ms": round(overall_stats["avg_processing_time"] or 0, 2),

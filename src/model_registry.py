...
class ModelRegistry:
    ...
    def promote_model_to_production(self, version: str) -> bool:
        ...
        try:
            ...
            if current_production:
                ...
                logger.info(
                    f"Archived previous production model version {current_production.version}"
                )
    ...
    def load_production_model(self):
        ...
        try:
            ...
            if not production_model:
                logger.warning(
                    "No production model found, attempting to load latest version"
                )
            ...
            logger.info(
                f"Loaded model version {production_model.version} from registry"
            )
    ...
    def compare_model_performance(self, version1: str, version2: str) -> Dict[str, Any]:
        ...
        try:
            ...
            comparison = {
                "version_1": {"version": version1, "metrics": v1_run.data.metrics},
                "version_2": {"version": version2, "metrics": v2_run.data.metrics},
            }

            # Calculate performance differences
            if "f1_score" in v1_run.data.metrics and "f1_score" in v2_run.data.metrics:
                f1_diff = (
                    v2_run.data.metrics["f1_score"] - v1_run.data.metrics["f1_score"]
                )
